...
\textbf{Interpretation}: Given identical demand rate for the two tasks, B ants spend less time performing tasks because the stimuli decrease more quickly (only task 1 is shown here, but would expect the same relation between A and B for task 2). There seems to be some \textbf{‘behavioral contagion’} (in Yuko’s terms), i.e., the A and B ants become more similar in behavior.}
